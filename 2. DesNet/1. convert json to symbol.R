...
#Setpath
input.path = "model/densenet-imagenet-169-0"
output.path = "2. DesNet/Created Code.R"

#load model
model = mx.model.load(input.path,125)
...
Create.Symbol.concat = function(LIST){
  ...
  parameter = paste0("dim", " = ", LIST$attr$dim, ", ",
                     "num.args", " = ", LIST$attr$num_args, ", ")
  
  INPUT = LIST$inputs 
  INPUT.num = sapply(1:length(INPUT), function(x){INPUT[[x]][1]+1})
   
    TEXT = paste0(LIST$name, ".list", " = list() \n", 
                    paste0(
                    LIST$name, ".list", " = append(", LIST$name, ".list", ", ",
                    Symbol.List$Names[INPUT.num], ") \n",collapse = ""),
                  LIST$name, " = mx.symbol.concat(", 
                  LIST$name, ".list", ", ",
                  parameter, 
                  "name = '", LIST$name, "'",
                  ")", "\n")
...
for (i in 1:length(Symbol.List$Names)){
  ...
  if ( LIST$op == "BatchNorm"){
    OUTPUT.file[i] = Create.Symbol.BatchNorm(LIST)
  }
  
  if ( LIST$op == "Concat"){
    OUTPUT.file[i] = Create.Symbol.concat(LIST)
  }
...
#have error at softmax preserve_shape parameter